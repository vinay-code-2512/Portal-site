...
export default function DesignationSelector({ value, onChange }: DesignationSelectorProps) {
  const { designations, loading, refresh } = useDesignations();
  ...
  async function handleAdd() {
    ...
    if (!trimmed) return;
    if (designations.some((d) => d.name.toLowerCase() === trimmed.toLowerCase())) {
      onChange(trimmed);
      setAdding(false);
      setNewName("");
      return;
    ...
    <div className="space-y-1.5">
      <div className="flex gap-2">
        <select
          ...
          disabled={loading}
        >
          <option value="">{loading ? "Loading..." : "Select designation"}</option>
          {designations.map((d) => (
